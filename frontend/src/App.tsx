  ...
  return (
    <section className="relative w-full min-h-screen sm:h-screen overflow-hidden">
      ...
      <nav className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between px-4 sm:px-6 md:px-10 py-4 sm:py-6">
        <div className="flex items-center gap-2 text-[#2d3a2a]">
          <span className="text-lg sm:text-xl md:text-2xl font-semibold tracking-tight">
            OriginESG<sup className="text-[10px] sm:text-xs font-medium">TM</sup>
          </span>
        </div>
      ...
      <div className="relative z-10 flex flex-col items-center text-center pt-24 sm:pt-28 md:pt-32 px-4 sm:px-6">
        ...
        >
          ...
          <span className="text-[#85AB8B]">
            raw data
            <br className="hidden sm:block" /> and verified emissions
          </span>
        </h1>
        <p className="mt-6 sm:mt-8 text-[#4b5b47] text-sm sm:text-base md:text-lg leading-relaxed max-w-md px-2">
          Ingest Scope 1, 2, and 3 emissions from any source — reviewed, approved, and locked for audit.
        </p>
      </div>

      {/* Dark gradient at bottom so text is always readable over the video */}
      <div className="absolute bottom-0 left-0 right-0 h-52 bg-gradient-to-t from-black/50 to-transparent z-[5] pointer-events-none" />
      ...
      <div className="absolute left-4 right-4 sm:right-auto sm:left-6 md:left-10 bottom-6 sm:bottom-8 md:bottom-10 z-10 max-w-sm">
        <div className="flex items-center gap-2 text-white mb-3">
          ...
          <span className="text-sm font-semibold">
            DataEngine<sup className="text-[10px]">TM</sup>
          </span>
        </div>
        <p className="text-white/85 text-xs leading-relaxed mb-6 max-w-xs">
          OriginESG ingests SAP, utility, and travel data, normalises everything to kg CO₂e, and creates an immutable audit trail your analysts can trust.
        </p>
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <button className="text-white text-sm font-semibold hover:opacity-80 transition-opacity">
            See How It Works.
          </button>
        </div>
      </div>
      ...
      <div className="hidden sm:flex absolute right-6 md:right-10 bottom-8 md:bottom-10 z-10 items-center gap-2 text-white/90 text-sm">
        ...
        <span className="font-medium">How we built this?</span>
